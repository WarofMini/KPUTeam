...
CScoreCount::CScoreCount(ID3D11DeviceContext * pContext)
: CUI(pContext)
, m_bStart(false)
{
...
CScoreCount::~CScoreCount(void)
{

}

CScoreCount * CScoreCount::Create(ID3D11DeviceContext * pContext)
{
	CScoreCount* pScoreCount = new CScoreCount(pContext);

	if (FAILED(pScoreCount->Initialize()))
		Safe_Release(pScoreCount);

	return pScoreCount;
}

HRESULT CScoreCount::Initialize(void)
{
...
_int CScoreCount::Update(const _float & fTimeDelta)
{
	if ((!m_bStart))
		return 0;
...
void CScoreCount::Release(void)
{
	CUI::Release();
}

HRESULT CScoreCount::Ready_Component(void)
{
	...
	//Texture
	pComponent = CResourcesMgr::GetInstance()->Clone_ResourceMgr(RESOURCE_STAGE, L"Texture_Count");
