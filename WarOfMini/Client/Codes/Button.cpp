...
CButtonUI::CButtonUI(ID3D11DeviceContext * pContext)
: CUI(pContext)
, m_bStart(false)
{
}
...
CButtonUI* CButtonUI::Create(ID3D11DeviceContext * pContext)
{
	CButtonUI* pUI = new CButtonUI(pContext);
...
HRESULT CButtonUI::Initialize(void)
{
...
_int CButtonUI::Update(const _float & fTimeDelta)
{
	if (!m_bStart)
		return 0;
...
void CButtonUI::Release(void)
{
	CUI::Release();
}

HRESULT CButtonUI::Ready_Component(void)
{
	...
	//Texture
	pComponent = CResourcesMgr::GetInstance()->Clone_ResourceMgr(RESOURCE_STAGE, L"Texture_Btn");
